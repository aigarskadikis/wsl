...
rm -rf /tmp/zabbix-release-6.4

# make fresh directory
mkdir -p /tmp/zabbix-release-6.4

# generate static session token by using menu "Users" => "API tokens"
SID=$(cat ~/.zabbix-6.4-auth)

# frontend endpoint
JSONRPC=$(cat ~/.zabbix-6.4-url)/api_jsonrpc.php

# download latest 6.4 branch from official repository of vendor
curl --insecure \
--location \
--output "/tmp/zabbix-release-6.4/6.4.zip" \
"https://git.zabbix.com/rest/api/latest/projects/ZBX/repos/zabbix/archive?at=refs%2Fheads%2Frelease%2F6.4&format=zip"

# unzip
cd "/tmp/zabbix-release-6.4"
unzip "6.4.zip"
rm -rf "/tmp/zabbix-release-6.4/6.4.zip"
