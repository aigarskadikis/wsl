...
SID=$(cat ~/.zabbix-6.0-auth)

# frontend endpoint
JSONRPC=$(cat ~/.zabbix-6.0-url)/api_jsonrpc.php

# download latest 6.0 branch from official repository of vendor
curl --insecure \
--location \
--output "/tmp/zabbix-release-6.0/6.0.zip" \
"https://git.zabbix.com/rest/api/latest/projects/ZBX/repos/zabbix/archive?at=refs%2Fheads%2Frelease%2F6.0&format=zip"

# unzip
cd "/tmp/zabbix-release-6.0"
unzip "6.0.zip"
rm -rf "/tmp/zabbix-release-6.0/6.0.zip"
